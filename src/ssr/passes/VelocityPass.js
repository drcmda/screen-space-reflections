...
export class VelocityPass extends Pass {
	#defaultMaterials = {}
	#velocityMaterials = {}
	#prevModelViewMatrix = new Matrix4()
	...
	#setVelocityMaterialInScene() {
		this._scene.traverse(c => {
			if (c.material) {
				...
				velocityMaterial.uniforms.prevProjectionMatrix.value =
					this.#prevProjectionMatrix

				if (c.userData.matrixWorldPrevious) {
					velocityMaterial.uniforms.prevModelViewMatrix.value.copy(
						c.userData.prevModelViewMatrix
					)
				}
	...
	#unsetVelocityMaterialInScene() {
		this._scene.traverse(c => {
			if (c.material) {
				if (c.userData.prevModelViewMatrix === undefined)
					c.userData.prevModelViewMatrix = new Matrix4()

				c.userData.prevModelViewMatrix.multiplyMatrices(
					this.#prevViewMatrix,
					c.matrixWorld
				)
